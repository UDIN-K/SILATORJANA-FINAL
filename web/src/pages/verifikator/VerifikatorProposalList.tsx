...
export function VerifikatorProposalList() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold text-slate-900">Semua Proposal</h2><p className="text-slate-500">Daftar proposal yang perlu diverifikasi.</p></div>
      <Card className="shadow-sm border-slate-200">
        <CardHeader className="p-4 border-b bg-slate-50/50">
          <div className="relative w-full sm:w-80"><Search className="absolute left-3 top-2.5 size-4 text-slate-400" /><Input placeholder="Cari..." className="pl-9 bg-white" value={search} onChange={e => setSearch(e.target.value)} /></div>
        </CardHeader>
            ...
            {filtered.map(item => (
              <div key={item.$id} className="flex items-center justify-between p-4 hover:bg-slate-50/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button size="sm" variant="outline" onClick={() => navigate(`/dashboard/verifikator/usulan/${item.$id}`)}><Eye className="size-4 mr-1" />Detail</Button>
                </div>
              </div>
            ))}
          </div>}
