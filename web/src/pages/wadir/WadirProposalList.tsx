        ...
        <CardContent className="p-0">
          ...
          <div className="divide-y divide-slate-100">{filtered.map(item => (
            <div key={item.$id} className="flex items-center justify-between p-4 hover:bg-slate-50/50">
              ...
              <div className="flex items-center gap-3"><StatusBadge status={item.status} /><Button size="sm" variant="outline" onClick={() => navigate(`/dashboard/wadir2/review/${item.$id}`)}><Eye className="size-4 mr-1" />Detail</Button></div>
            </div>
          ))}</div>}
        </CardContent>
      </Card>
